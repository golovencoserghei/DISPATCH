...
// Держит WebSocket к локальному MCP-серверу, принимает команды, исполняет их
// в вкладке с доступом через chrome.scripting (DOM/eval) и chrome.debugger/CDP
// (скриншоты, перехват сети/консоли, эмуляция, точный ввод).
...
const PROTOCOL_VERSION = 1; // должен совпадать с сервером (bridge.ts)
...
function connect() {
  ...
  ws.onopen = () => {
    ...
    send({ kind: "event", event: "hello", data: { name: "Dispatch", protocolVersion: PROTOCOL_VERSION, token: state.token || "", ua: navigator.userAgent, ts: Date.now() } });
    updateBadge();
  };
  ws.onclose = () => {
...
function handleCdpEvent(method, params) {
  switch (method) {
    ...
    case "Log.entryAdded": {
      ...
      pushConsole({ kind: "log", level: e.level, text: e.text, url: e.url, line: e.lineNumber, ts: Date.now() });
      break;
    }
  }
}
...
chrome.debugger.onEvent.addListener((source, method, params) => {
  ...
  try { handleCdpEvent(method, params); } catch { /* защита буфера */ }
});
  ...
  (async () => {
    switch (msg.type) {
      ...
      case "grantActive": {
        ...
        if (!tab) { sendResponse({ ok: false, error: "нет активной вкладки" }); break; }
        await grantAccess(tab);
        pushLog(`доступ выдан: ${tab.title}`);
        sendResponse({ ok: true, grantedTabId: tab.id, grantedTitle: tab.title });
        break;
      }
      case "revokeAccess":
      ...
      case "reconnect":
        ...
        break;
      default:
        sendResponse({ ok: false, error: "неизвестное сообщение popup" });
    }
  })();
...
// ── старт / поддержание живости ──────────────────────────────────────────────────
chrome.alarms.create("dispatch-keepalive", { periodInMinutes: 0.4 });
chrome.alarms.onAlarm.addListener((a) => {
  if (a.name === "dispatch-keepalive" && state.enabled && !state.connected) connect();
});
